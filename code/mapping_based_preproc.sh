...
genome_ref="../references/Mus_musculus.NCBIM37.67.dna.toplevel.fa"
genome_ref_gtf="../references/Mus_musculus.NCBIM37.67.gtf"

# check if HISAT2 index files exist for reference genome
if [ -f ${genome_ref}.1.ht2 ]; then
  echo "HISAT2 index files already exist."
else
  echo "HISAT2 index files not found. Generating index..."
  hisat2-build -p ${threads} ${genome_ref} ${genome_ref}
...
do
    ...
    hisat2 -p ${threads} --dta -x ${genome_dir} -1 ${R1} -2 ${R2} -S ${SAM}
...
do
    ...
    stringtie -p ${threads} -G ${genome_ref_gtf} -o ${GTF} ${BAM_M}
done
